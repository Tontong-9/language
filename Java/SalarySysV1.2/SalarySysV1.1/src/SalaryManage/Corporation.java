...
public class Corporation {
	...
	public void  saveAsBinaryFile(String filename) {
		...
		try {
			...
			out.writeInt(nextId);//先写入职员的总数
			for(int i = 0;i < nextId;i++) {
				String employeemsg = employees[i].toString();
				byte[] msg = employeemsg.getBytes();
				System.out.println(""+i+":"+msg.length);
	...
	public void  saveAsTextFile(String filename) {
		...
		try {
			   ...
			   for(int i = 0 ; i < nextId ; i++) {
				   out.println(employees[i].toString());

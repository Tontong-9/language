...
public class Corporation_HashSet {

	private String corpName;  //公司名
	private String corpDescript;  //公司业务描述
	private String superManager;  //公司法人
	
	private  HashSet<Employee>  employees; //用来存放公司的员工对象
	
    private  Scanner  scan; //由于很多方法都需要键盘输入数据，所以将scan对象放在类成员使用

	public static void main(String[] args) {
		
		Corporation_HashSet corp = new Corporation_HashSet("南航科院软件有限公司", "专门从事企业级应用开发","郭老师");
	...
	Corporation_HashSet(String name , String des , String superManager ){
		corpName = name;
		corpDescript = des;
		this.superManager = superManager;
		employees = new HashSet<Employee>(100);
	...
	private void initEmployee() {
		...
		Employee employee07 = new Employee("张七",2019,6,28,18000.0,"校长","444");
		employee07.setId(6);
		employees.add(employee07);
		System.out.println("职工数据新版本初始化成功！！！");
	}
	...
	/**
	 * 由于set没有实现排序的接口，所以转化为链表集合排序
	 */
	public void  sortEmployeeBySalary() {
		
	    LinkedList<Employee> datas = new LinkedList<Employee>(employees);
	    datas.sort(Comparator.comparing(Employee::getSalary).reversed());
	...
	public void sortEmployeeByHireTime() {
		  LinkedList<Employee> datas = new LinkedList<Employee>(employees);
		    datas.sort(Comparator.comparing(Employee::getHireDay).reversed());
		    for(Employee e : datas) {
		    	System.out.println(e.toString());
		    }
	}
    ...
    public  void  sortEmployeeByDayAndSalary() {
    	 ...
    	 LinkedList<Employee> datas = new LinkedList<Employee>(employees);
    	 datas.sort(Comparator.comparing(Employee::getHireDay).thenComparing(Employee::getSalary));
    	 for(Employee e : datas) {
		    	System.out.println(e.toString());
		 }   
    }
